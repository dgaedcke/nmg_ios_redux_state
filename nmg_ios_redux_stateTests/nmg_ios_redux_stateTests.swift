...
//		print("end store.dispatch")
	...
	func test_adding_teams() {
		...
		let teams = makeTeams(count: TestConst.TEST_TEAM_COUNT)
		for t in teams {
...
//		callbackCount += 1
		...
		case "test_adding_teams":
			...
			if teams.count == TestConst.TEST_TEAM_COUNT {
				let filteredTeams = teams.filter( { $0.id == "000"} )
				XCTAssert(filteredTeams.count == 1, "missing a team")
				self.pendExpectation?.fulfill()
				XCTAssert(teams.count == TestConst.TEST_TEAM_COUNT, "Team count \(teams.count) (should be \(TestConst.TEST_TEAM_COUNT)")
			}
		...
		// DO NOT unsubscribe after each test callback
		// because first callback is empty state
	}
}
